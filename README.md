...
Everything here is fabricated. The six students (Alice, Bob, Carol, Dave, Elizabeth, Fred) do not exist, their GitHub handles (`ghclass-alice` through `ghclass-fred`) do not correspond to real accounts, and the assignment content is intentionally generic placeholder text. No user was ever invited to this organization and no collaborator was ever added to a repository.
...
- `Grading/` holds one collection and feedback script per assignment (`hw1.R`, `midterm1.R`). Running a script generates its working folder (cloned submissions, a scores CSV, and scaffolded comment files); those folders hold student work and grades, so they stay out of version control and are not part of this repository.
